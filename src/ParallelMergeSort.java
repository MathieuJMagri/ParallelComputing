...
//      + Runtime.getRuntime().availableProcessors() + 
  ...
  public static void parallelMergeSort(int[] list) {
    ...
    ForkJoinPool pool = new ForkJoinPool(NUMPROCESSORS);    // Create a ForkJoinPool with specified # of processors.
//    ForkJoinPool pool = new ForkJoinPool();  // Create a ForkJoinPool with all available processors.
  ...
  private static class SortTask extends RecursiveAction {
    private final int THRESHOLD = 500;		//Default threshold: 500
    private int[] list;
    ...
    protected void compute() {
      if (list.length < THRESHOLD)
        java.util.Arrays.sort(list); //if it is under threshold no point in using forkjoin
      else {
