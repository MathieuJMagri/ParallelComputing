...
// This version of DiningPhilosophers fixes the deadlock but no starvation.
// Starvation is noticeable by looking at which philosophers finish eating first i.e. who arrives at
// 50 meals first.
public class DiningPhilosophersNoDeadlock {
  ...
  public static void main(String[] args) {
    ...
    // for i < philosopher number -> label chopsticks for each philosopher and then
    // start thread
    for (int i = 0; i < philosophers.length; i++) {

      // To fix the deadlock, we must make one of the philosophers start by taking the
      // right chopstick
      // This is done by the 0th philosopher.
      if (i == 0) {
        philosophers[i] = new Philosopher(chopsticks[i % (numberOfPhilosophers)],
            chopsticks[(i + 1) % numberOfPhilosophers], i, 0);
      } else {
        philosophers[i] = new Philosopher(chopsticks[(i + 1) % numberOfPhilosophers],
            chopsticks[i % (numberOfPhilosophers)], i, 0);
      }
  ...
  // A Philosopher can do one of three things, it can either be thinking, waiting
  // or eating.
  public static class Philosopher implements Runnable {
    ...
    public void run() {
      ...
      for (int i = 0; i < 50; i++) {
        ...
        if (i == 49) {
          System.out
              .println("Philosopher #" + this.id + " has eaten " + this.numberOfMeals + " times");
        }
  ...
  public static class Chopstick {
    ...
    // By putting false, the lock will be awarded arbitrarily, this is to create a
    // starvation effect which will be corrected.
